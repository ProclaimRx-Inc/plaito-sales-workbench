...
export const TIERS: Exclude<Tier, null>[] = ["Tier_A", "Tier_A_Colocated", "Tier_B", "Tier_Plus"]
export const tierLabel = (t: Tier) => (t === "Tier_A" ? "Priority (A)" : t === "Tier_A_Colocated" ? "Co-located" : t === "Tier_B" ? "B" : t === "Tier_Plus" ? "Plus" : "Untargeted")
...
function ix() {
  ...
  for (const r of data.rx) {
    ...
    if (r.brandTrx > 0 && !firstWriteWeek.has(r.npi)) firstWriteWeek.set(r.npi, r.weekIndex)
  }
...
// ---- weekly trend ---------------------------------------------------------------
export type TrendRow = { week: string; weekIndex: number; brandTrx: number; brandNrx: number; starts: number; cont: number; writers: number; newWriters: number; holiday?: string }
...
export function weeklyTrend(): { rows: TrendRow[]; def: Definition } {
  ...
  for (const w of data.weeks.slice(data.launchWeekIndex)) {
    ...
    let brand = 0, nrx = 0, starts = 0, cont = 0, writers = 0, nw = 0
    for (const r of rs) {
      brand += r.brandTrx; nrx += r.brandNrx; starts += r.brandStarts; cont += r.brandCont
      if (r.brandTrx > 0) { writers++; if (firstWriteWeek.get(r.npi) === w.index) nw++ }
    }
    rows.push({ week: w.end, weekIndex: w.index, brandTrx: brand, brandNrx: nrx, starts, cont, writers, newWriters: nw, holiday: w.holiday })
  }
  return {
    rows,
    def: {
...
export function headline() {
  ...
  const avg4 = sum(last4.map((r) => r.brandTrx)) / 4
  const avgPrior4 = sum(prior4.map((r) => r.brandTrx)) / 4
  const best = rows.reduce((a, b) => (b.brandTrx > a.brandTrx ? b : a))
  const sinceLaunch = sum(rows.map((r) => r.brandTrx))
  const sinceLaunchNrx = sum(rows.map((r) => r.brandNrx))
  const writersEver = new Set(data.rx.filter((r) => r.brandTrx > 0).map((r) => r.npi)).size
  const active4 = new Set(data.rx.filter((r) => r.brandTrx > 0 && r.weekIndex > data.latestWeek.index - 4).map((r) => r.npi)).size
  return { last, prev, wow: last.brandTrx - prev.brandTrx, wowPct: (last.brandTrx - prev.brandTrx) / prev.brandTrx, avg4, avg4Growth: (avg4 - avgPrior4) / avgPrior4, best, sinceLaunch, sinceLaunchNrx, writersEver, active4 }
}
...
export function wowBridge(weekIndex = d().latestWeek.index): { buckets: BridgeBucket[]; movers: { npi: string; name: string; territory: string; prev: number; cur: number; delta: number; bucket: string }[]; def: Definition } {
  ...
  const cur = new Map((rxByWeek.get(weekIndex) ?? []).map((r) => [r.npi, r.brandTrx]))
  const prev = new Map((rxByWeek.get(weekIndex - 1) ?? []).map((r) => [r.npi, r.brandTrx]))
  ...
  const desc: Record<string, string> = {
    "New writers": "First week ever with tunits > 0",
  ...
  return {
    ...
    def: {
      ...
      notes: ["This answers 'why did TRx move this week' the same way every week, instead of an ad-hoc narrative."],
    },
  }
}
...
export function writerLifecycle(asOf = d().latestWeek.index): { rows: WriterRow[]; counts: Record<WriterSegment, number>; def: Definition } {
  ...
  for (const [npi, rs] of rxByNpi) {
    const written = rs.filter((r) => r.brandTrx > 0 && r.weekIndex <= asOf)
    ...
    const last13 = sum(written.filter((r) => r.weekIndex > asOf - 13).map((r) => r.brandTrx))
    ...
    rows.push({
      ...
      firstWeek: first, firstWeekEnd: data.weeks[first].end, lastWeek: last, weeksWritten: written.length, last4, last13, sinceLaunch: sum(written.map((r) => r.brandTrx)), segment,
...
export function newWriterReport(weekIndex: number) {
  ...
  const rows = life.rows
    ...
    .map((r) => {
      ...
      return { ...r, tunits: wk.brandTrx, starts: wk.brandStarts, channel: (Object.keys(wk.channel) as PayerChannel[]).filter((c) => wk.channel[c] > 0).map((c) => `${c} ${wk.channel[c]}`).join(", ") }
    })
...
export function cohortRetention() {
  ...
  const { rxByNpi } = ix()
  const wroteAgainWithin = (r: WriterRow, weeks: number) => (rxByNpi.get(r.npi) ?? []).some((x) => x.brandTrx > 0 && x.weekIndex > r.firstWeek && x.weekIndex <= r.firstWeek + weeks)
...
export type TerritoryRow = {
  ...
  hcps: number; targets: number; priorityA: number; mktTrx13: number; brandTrx13: number; share13: number; channel13: Record<PayerChannel, number>
...
export function territoryScorecard(weeks = 13): { rows: TerritoryRow[]; def: Definition } {
  ...
  for (const t of data.territories) {
    byT.set(t.code, {
      ...
      hcps: 0, targets: 0, priorityA: 0, mktTrx13: 0, brandTrx13: 0, share13: 0, channel13: { Commercial: 0, Medicaid: 0, Medicare: 0, TriCare: 0, Cash: 0 },
  ...
  for (const h of data.hcps) {
    ...
    if (h.tier === "Tier_A") row.priorityA++
  }
  for (const r of data.rx) {
    const h = ix().hcpByNpi.get(r.npi)!
    const row = byT.get(h.territoryCode)!
    if (r.weekIndex >= from) {
      row.mktTrx13 += r.mktTrx; row.brandTrx13 += r.brandTrx
    ...
    if (qWeeks.has(r.weekIndex)) row.actualQ += r.brandTrx
  }
  for (const c of data.calls) {
  ...
  for (const row of byT.values()) {
    ...
    row.share13 = row.mktTrx13 ? row.brandTrx13 / row.mktTrx13 : 0
    row.attainment = row.goalQ ? row.actualQ / row.goalQ : NaN
  }
  const rows = [...byT.values()].sort((a, b) => (a.whiteSpace ? 1 : 0) - (b.whiteSpace ? 1 : 0) || b.brandTrx13 - a.brandTrx13)
  return {
    rows,
    def: {
...
export function territoryTrend(code: string) {
  ...
  const byWeek = new Map<number, { brand: number; mkt: number }>()
  for (const r of data.rx) {
    if (!npis.has(r.npi) || r.weekIndex < data.launchWeekIndex) continue
    const v = byWeek.get(r.weekIndex) ?? { brand: 0, mkt: 0 }
    v.brand += r.brandTrx; v.mkt += r.mktTrx
    byWeek.set(r.weekIndex, v)
  }
  return data.weeks.slice(data.launchWeekIndex).map((w) => ({ week: w.end, brandTrx: byWeek.get(w.index)?.brand ?? 0, mktTrx: byWeek.get(w.index)?.mkt ?? 0 }))
}

export function territoryTopHcps(code: string, n = 10) {
  const data = d()
  const from = data.latestWeek.index - 12
  const acc = new Map<string, { brand: number; mkt: number }>()
  for (const r of data.rx) {
    if (r.weekIndex < from) continue
    const h = ix().hcpByNpi.get(r.npi)!
    if (h.territoryCode !== code) continue
    const v = acc.get(r.npi) ?? { brand: 0, mkt: 0 }
    v.brand += r.brandTrx; v.mkt += r.mktTrx
    acc.set(r.npi, v)
  }
  return [...acc.entries()].map(([npi, v]) => ({ npi, name: hcpName(hcp(npi)), specialty: hcp(npi).specialty, tier: hcp(npi).tier, ...v })).sort((a, b) => b.brand - a.brand || b.mkt - a.mkt).slice(0, n)
}
...
export type OpportunityRow = {
  ...
  brand13: number; mkt13: number; branded13: number; buspirone13: number; auvelity13: number; share13: number
...
export function hcpOpportunity(): { rows: OpportunityRow[]; medians: { potential: number; current: number }; def: Definition } {
  ...
  const asOf = data.latestWeek.index
  const from = asOf - 12
  const acc = new Map<string, { brand: number; mkt: number; br: number; bu: number; au: number }>()
  for (const r of data.rx) {
    if (r.weekIndex < from) continue
    const v = acc.get(r.npi) ?? { brand: 0, mkt: 0, br: 0, bu: 0, au: 0 }
    v.brand += r.brandTrx; v.mkt += r.mktTrx; v.br += r.mktBranded; v.bu += r.buspirone; v.au += r.auvelity
    acc.set(r.npi, v)
  }
  ...
  const currents = hcps.map((h) => acc.get(h.npi)!.brand)
  ...
  const rows: OpportunityRow[] = hcps.map((h, i) => {
    ...
    else if (quadrant === "Convert" && c13 === 0) { action = "Schedule first call"; reason = `Top-half market volume (${v.mkt} MDD TRx / 13 wk), no Escitalopram, zero calls in 13 weeks` }
    else if (quadrant === "Convert" && status === "Lapsed writer") { action = "Re-engage lapsed writer"; reason = `Wrote Escitalopram before, none in ${s === "Lapsed" ? "8+" : "4–7"} weeks; market volume still high` }
    else if (quadrant === "Convert") { action = "Add copay/starter detail"; reason = `Called ${c13}× in 13 weeks but no Escitalopram; ${v.bu > 0 ? "buspirone writer" : "branded writer"} profile matches current Escitalopram writers` }
    else if (quadrant === "Grow") { action = "Maintain frequency"; reason = `Writing ${v.brand} TRx / 13 wk with share ${((v.brand / Math.max(1, v.mkt)) * 100).toFixed(1)}% of a large market` }
    else if (quadrant === "Defend") { action = "Protect: check access"; reason = "Writing Escitalopram but small total market; watch for lapse" }
    else { action = "Deprioritize"; reason = "Bottom-half market volume and no Escitalopram activity" }
    return {
      ...
      brand13: v.brand, mkt13: v.mkt, branded13: v.br, buspirone13: v.bu, auvelity13: v.au, share13: v.mkt ? v.brand / v.mkt : 0,
...
export function targetCoverage(quarter = d().latestWeek.quarter) {
  ...
  const wrote = new Set(data.rx.filter((r) => weeks.has(r.weekIndex) && r.brandTrx > 0).map((r) => r.npi))
...
export function eventPrePost(entries: { npi: string; date: string }[], horizon = 8) {
  ...
  const rows = entries.map((e) => {
    ...
    const pre = sum(rs.filter((r) => r.weekIndex < wk && r.weekIndex >= wk - horizon).map((r) => r.brandTrx))
    const post = sum(rs.filter((r) => r.weekIndex >= wk && r.weekIndex < wk + horizon).map((r) => r.brandTrx))
...
export function channelSplit(key: WindowKey) {
  ...
  for (const r of data.rx) {
    if (!weeks.has(r.weekIndex) || r.brandTrx === 0) continue
    for (const c of CHANNELS) {
      const n = r.channel[c]
      if (!n) continue
      const f = n / r.brandTrx
      acc[c].trx += n; acc[c].starts += r.brandStarts * f; acc[c].cont += r.brandCont * f; acc[c].nrx += r.brandNrx * f
    }
  }
...
export function copaySummary() {
  ...
  return { totals: { claims: claims.length, net: net.length, reversals, network, networkShare: network / net.length, patients, multiFill, starter, spend: sum(net.map((c) => c.copay)), avgOop: sum(net.map((c) => c.oop)) / net.length }, pharmacies, weekly, def }
}
